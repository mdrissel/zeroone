...
def walk_forward_backtest(
    ...
    Returns extended dict including:
      daily_returns_net: list of daily net returns (parallel to state_series)
      state_series: list of regime state at each backtest step
      date_series: list of pd.Timestamp dates at each step
    These enable per-regime analytics (autocorrelation, Sortino) in the caller.
    """
    ...
    nan_result = {
        ...
        "daily_returns_net": [], "state_series": [], "date_series": [],
        "cost_bps": cost_bps,
    }
    ...
    strategy_returns_gross: list[float] = []
    strategy_returns_net: list[float] = []
    state_series: list[int] = []
    ...
    for t in range(min_train, len(labels) - 1):
        ...
        date_series.append(labels.index[t])
    ...
    return {
        ...
        "bh_sharpe": bh_sharpe,
        "excess_sharpe": (
            float(net_sharpe - bh_sharpe)
            if np.isfinite(net_sharpe) and np.isfinite(bh_sharpe)
            else float("nan")
        ),
        ...
        "cost_bps": cost_bps,
        # Extended fields for per-regime analytics
        "daily_returns_net": strategy_returns_net,
